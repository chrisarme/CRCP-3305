...
// when an ofEasyCamMod is moving due to momentum, this keeps it
...
//----------------------------------------
void ofEasyCamMod::update(ofEventArgs & args) {
...
//----------------------------------------
void ofEasyCamMod::begin(ofRectangle _viewport) {
	if (!bEventsSet) {
		setEvents(ofEvents());
	}
...
//----------------------------------------
void ofEasyCamMod::setTarget(const ofVec3f& targetPoint) {
	target.setPosition(targetPoint);
	lookAt(target);
}

//----------------------------------------
void ofEasyCamMod::setTarget(ofNode& targetNode) {
	target = targetNode;
	lookAt(target);
}

//----------------------------------------
ofNode& ofEasyCamMod::getTarget() {
	return target;
}

//----------------------------------------
void ofEasyCamMod::setDistance(float distance) {
	setDistance(distance, true);
}

//----------------------------------------
void ofEasyCamMod::setDistance(float distance, bool save) {//should this be the distance from the camera to the target?
	if (distance > 0.0f) {
		if (save) {
...
//----------------------------------------
float ofEasyCamMod::getDistance() const {
	return target.getPosition().distance(getPosition());
}

//----------------------------------------
void ofEasyCamMod::setAutoDistance(bool bAutoDistance) {
...
//----------------------------------------
void ofEasyCamMod::setDrag(float drag) {
	this->drag = drag;
}

//----------------------------------------
float ofEasyCamMod::getDrag() const {
	return drag;
}

//----------------------------------------
void ofEasyCamMod::setTranslationKey(char key) {
	doTranslationKey = key;
}

//----------------------------------------
char ofEasyCamMod::getTranslationKey() {
	return doTranslationKey;
}

//----------------------------------------
void ofEasyCamMod::enableMouseInput() {
	if (!bMouseInputEnabled && events) {
		ofAddListener(events->update, this, &ofEasyCamMod::update);
		ofAddListener(events->mouseDragged, this, &ofEasyCamMod::mouseDragged);
		ofAddListener(events->mousePressed, this, &ofEasyCamMod::mousePressed);
		ofAddListener(events->mouseReleased, this, &ofEasyCamMod::mouseReleased);
		//ofAddListener(events->mouseScrolled, this, &ofEasyCamMod::mouseScrolled);
	}
...
//----------------------------------------
void ofEasyCamMod::disableMouseInput() {
	if (bMouseInputEnabled && events) {
		ofRemoveListener(events->update, this, &ofEasyCamMod::update);
		ofRemoveListener(events->mouseDragged, this, &ofEasyCamMod::mouseDragged);
		ofRemoveListener(events->mousePressed, this, &ofEasyCamMod::mousePressed);
		ofRemoveListener(events->mouseReleased, this, &ofEasyCamMod::mouseReleased);
		//ofRemoveListener(events->mouseScrolled, this, &ofEasyCamMod::mouseScrolled);
	}
...
//----------------------------------------
void ofEasyCamMod::setEvents(ofCoreEvents & _events) {
...
//----------------------------------------
bool ofEasyCamMod::getMouseInputEnabled() {
	return bMouseInputEnabled;
}

//----------------------------------------
void ofEasyCamMod::enableMouseMiddleButton() {
	bEnableMouseMiddleButton = true;
}

//----------------------------------------
void ofEasyCamMod::disableMouseMiddleButton() {
	bEnableMouseMiddleButton = false;
}

//----------------------------------------
bool ofEasyCamMod::getMouseMiddleButtonEnabled() {
	return bEnableMouseMiddleButton;
}

//----------------------------------------
void ofEasyCamMod::updateTranslation() {
	if (bApplyInertia) {
		moveX *= drag;
		moveY *= drag;
		moveZ *= drag;
...
//----------------------------------------
void ofEasyCamMod::updateRotation() {
	//if (bApplyInertia) {
	//	xRot *= drag;
	//	yRot *= drag;
	//	zRot *= drag;
...
void ofEasyCamMod::mousePressed(ofMouseEventArgs & mouse) {
...
void ofEasyCamMod::mouseReleased(ofMouseEventArgs & mouse) {
...
void ofEasyCamMod::mouseDragged(ofMouseEventArgs & mouse) {
	mouseVel = mouse - lastMouse;

	updateMouse(mouse);
}

void ofEasyCamMod::mouseScrolled(ofMouseEventArgs & mouse) {
	ofRectangle viewport = getViewport(this->viewport);
	prevPosition = ofCamera::getGlobalPosition();
	prevAxisZ = getZAxis();
...
void ofEasyCamMod::updateMouse(const ofMouseEventArgs & mouse) {
	ofRectangle viewport = getViewport(this->viewport);
	int vFlip;
	if (isVFlipped()) {
		vFlip = -1;
